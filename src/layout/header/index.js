...
const Index = () => {
  ...
  const showModal = () => {
    if (token) {
      ...
      message.warning("You are not Logged in, please login First!");//message
    }
  };
  ...
  return (
    <div className="flex justify-between">
      ...
      <div className="flex gap-5 items-center">
        ...
        {/* to show login and signup when not login that is when we don't have token */}
        {!token &&
          Auth?.map((item) => (
            <div
              key={item.link}
          ...
          <div>
          <div className="text-white font-bold">{name}</div> {/* to show name when logged */}
        </div>
      </div>
    </div>
  );
